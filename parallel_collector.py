...
async def collect_marketplace(client, data_dir, label, force=False):
    ...
    async def collect_list(name, endpoint, state_filter, extract_key,
                           ...
                           accept="application/json", max_count=5000,
                           custom_body=None):
    ...
    entity_coros = [
        ...
        collect_list_throttled("sb_themes", "/sb/themes/list", ["enabled", "paused"], "themes",
                               ...
                               max_count=100),
        # SD Entities (3)
        collect_list_throttled("sd_campaigns", "/sd/campaigns", ["enabled", "paused"], "campaigns", method="GET", max_count=5000),
        collect_list_throttled("sd_ad_groups", "/sd/adGroups", ["enabled", "paused"], "adGroups", method="GET", max_count=5000),
        collect_list_throttled("sd_targets", "/sd/targets", ["enabled", "paused"], "targets", method="GET", max_count=5000),
    ]
