...
BASE_DIR = Path(os.environ.get("PPC_BASE_DIR", "."))
...
def preflight_check(today):
    ...
    for prefix in agent1_kritik:
        ...
        if not fpath.exists():
            ...
            logger.warning("  ✗ Agent 1 verisi eksik: %s_%s.json", today, prefix)
...
def save_error_log(hata_tipi, hata_mesaji, traceback_str=None, adim=None, extra=None):
    """
    ...
    Parametreler:
        hata_tipi    : Hata sinifi veya kategori (ör. "API_ERROR", "EntityNotFound", "VerificationFailed")
        hata_mesaji  : Hata aciklamasi
        traceback_str: traceback.format_exc() ciktisi (beklenmeyen Python hatalarinda)
        adim         : Hatanin gerceklestigi adim (ör. "bid_execution", "preflight_check")
        extra        : Ek baglam bilgisi dict olarak (ör. {"kampanya": "X", "api_endpoint": "Y"})
    """
    log_dir = LOG_DIR
    log_dir.mkdir(parents=True, exist_ok=True)
    ...
    if traceback_str:
        kayit["traceback"] = traceback_str[:1000]
    if extra:
        kayit["extra"] = extra
...
def run_executor(today=None, force_execute=False):
    """
    ...
    logger.info("=== AGENT 3 v2 EXECUTOR BASLADI — %s ===", today)
...
if __name__ == "__main__":
    ...
    if "--verify" in sys.argv:
        # Dogrulama modu: rollback log'u oku, _verify_ dosyalarindan dogrula
        ...
        if not rollback_path.exists():
            ...
            # _verify_ dosyalari varsa otomatik dogrulama yap
            # Verify dosyalarinin prefix'i: rollback tarihindeki data_date
            # veya --data-date parametresi
            data_date = today
            for i, arg in enumerate(sys.argv):
            ...
            if verify_test_file.exists():
                # _verify_ dosyalari mevcut — otomatik dogrulama
                ...
                # _verify_ dosyalari henuz yok — gorevleri goster
                print(json.dumps(report, indent=2, ensure_ascii=False))
                print(f"\n--- Dogrulama icin once verify verilerini cekin: ---")
                print(f"--- MCP tool: amazon_ads_collect_verify_data (data_date={data_date}) ---")
                print(f"--- Sonra tekrar --verify calistirin. ---")
    else:
        force = "--execute" in sys.argv
        result = run_executor(today=custom_date, force_execute=force)
